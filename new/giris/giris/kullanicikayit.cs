...
namespace giris
{
    public partial class kullanicikayit : Form
    {
        ...
        private void button_kaydey_Click(object sender, EventArgs e)
        {
            ...
            if ((text_tc.Text.Length == 11) && (text_sifre.Text == text_sifretekrar.Text) && (text_tel.Text.Length == 7) && (kontrol == true))
            {
                ...
                kullanicikaydet.CommandText = "insert into tbl_kullanicikimlik(k_adi,k_sadi,k_cinsiyet,k_mail,k_tel,k_dtarihi,k_dyeri,k_tcno,kullanici_no) values(@kuladi,@kulsadi,@kulcins,@mail,@kultel,@kuldogtar,@kuldogyer,@kultc,@kullanicino)";
                sifrekaydet.Parameters.AddWithValue("@kullaniciadi", text_kadi.Text);
                sifrekaydet.Parameters.AddWithValue("@sifre", text_sifre.Text);
                sifrekaydet.CommandText = "insert into tbl_kullanici(k_adi,sifre) values(@kullaniciadi,@sifre)";
